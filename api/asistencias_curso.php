...
// Info del curso
$infoCurso = $conexion->query("SELECT id_curso, nombre_curso FROM cursos WHERE id_curso = $idCurso")->fetch_assoc();
if (!$infoCurso) {
    header('Location: menu.php');
    exit;
}

// Permisos: Admin y Preceptor pueden editar; Docente/Alumno solo ver
$puedeEditar = ($perfilActual === 4 || $perfilActual === 5);

// Listado de alumnos del curso
$alumnos = $conexion->query("SELECT a.id_alumno, u.apellido, u.nombre,
                                (SELECT 1 FROM notas n 
                                 INNER JOIN curso_materia cm ON n.id_curso_materia = cm.id_curso_materia
                                 WHERE n.id_alumno = a.id_alumno AND cm.id_curso = $idCurso AND n.id_estado = 2 LIMIT 1) as es_recursante
                               FROM alumnos a
...
<head>
  ...
  <style>
    ...
    .badge-condicion { display:inline-flex; align-items:center; justify-content:center; width:18px; height:18px; border-radius:50%; font-size:10px; font-weight:bold; flex-shrink:0; margin-right:6px; line-height:1; }
    .badge-c { background:#999; color:#fff; }
    .badge-r { background:#000; color:#fff; }
...
<body>
  ...
  <div class="container">
    <div class="titulo-bar">
      <h2 class="titulo"><i class="fa fa-user-check"></i> Asistencia por Curso (<?php echo htmlspecialchars($infoCurso['nombre_curso']); ?>)</h2>
      <a href="miscursos.php" class="btn-volver"><i class="fa fa-arrow-left"></i> Volver a Mis Cursos</a>
    </div>

    <?php if (!$puedeEditar && $perfilActual !== 1): ?>
      <div class="badge-ok" style="background:#fde2e2; color:#a10000; margin-bottom:8px;"><i class="fa fa-triangle-exclamation"></i> No tenés permisos para editar este curso.</div>
    <?php endif; ?>
          ...
          <tr data-alumno="<?php echo (int)$al['id_alumno']; ?>">
            ...
            <td class="sticky-nombre" style="text-align:left; width:270px;">
              <div class="nombre-wrap">
                ...
                <?php if (intval($al['es_recursante'] ?? 0) === 1): ?>
                  <span class="badge-condicion badge-r" title="Recursante">R</span>
                <?php else: ?>
                  <span class="badge-condicion badge-c" title="Cursa">C</span>
                <?php endif; ?>
              </div>
            </td>
  ...
  <script>
    ...
    window.saveHeaderDateModal = async function(){
      ...
      try{
        ...
        /* Deshabilitado para curso: edición de fecha no soportada en AJAX de curso
        const res = await fetch('asistencias_curso_ajax.php?action=cambiar_fecha_sesion', {
          method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: params.toString()
        });*/
        let json = null;
        try { json = { ok:false }; } catch(_) { json = null; }
        console.log('RESP cambiar_fecha_sesion (disabled)');
        if (true) {
    ...
    const LS_KEY_FECHA = `asist_fecha_curso_${ID_CURSO}`;
    ...
    async function cargarTodasAsistencias() {
      ...
      try{
        const res = await fetch('asistencias_curso_ajax.php?action=listar_todo', {
          method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
          body: new URLSearchParams({ id: ID_CURSO, fecha: inpFecha.value })
        });
    ...
    async function cargarAsistencias() {
      ...
      try{
        const res = await fetch('asistencias_curso_ajax.php?action=listar_todo', {
          method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
          body: new URLSearchParams({ id: ID_CURSO, fecha })
        });
        const json = await res.json();
        const sesiones = Array.isArray(json.sesiones) ? json.sesiones : [];
        const data = json.data || {};
    ...
    function renderSesiones(sesiones, data){
      ...
      // Deshabilitar edición por doble clic en headers para curso
    ...
    async function setEstado(idAlumno, sesion, fecha, estado){
      if(!fecha) return;
      try{
        const res = await fetch('asistencias_curso_ajax.php?action=guardar', {
          method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
          body: new URLSearchParams({ id: ID_CURSO, alumno: idAlumno, fecha, sesion, estado })
        });
    ...
    // Navegación tipo Excel para selects de asistencia (A/P)
    (function(){
      function findTd(el){
        let n = el;
      ...
      function moveHorizontal(selectEl, dir){
        ...
        while (cur) {
          ...
          const sel = cur.querySelector && cur.querySelector('select.sel-estado');
          if (sel) { sel.focus(); return; }
        }
      }
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if (!(el && el.tagName === 'SELECT' && el.classList.contains('sel-estado'))) return;

        const key = e.key;

        // Escribir A/P fija el valor
